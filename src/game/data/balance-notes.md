...
|---|---|---|---|
| 1 | 8 | 30 + 15 fogwort | 60 + 15 tinder |
| 2 | 16 | 80 + 30 fogwort | 140 + 10 resin |
...
Items completion is a **completionist book**, not live occupancy. Hollow N/MAX is unique held stacks; “N of 137 known” is Times Found. The HUD always shows both: `Known 6/137` next to `Hollow 5/12`. Category tabs follow known, not occupancy — dumping the last loaf leaves a Food tab with a `known-empty` qty-0 tile.
...
“The Work Went On”. Ample-fuel full-span still lights it. Claim of a
fuel-halt recap clears the dry action (`actions.active`) so Camp / Skills
do not still show Tend queued — Melvor sets `isActive` false on halt,
not on the next failed tick. The starve that printed `out of Tinderscrap`
also increments `tinderHalts` (live dry-halt’s counter) so “Out of Tinder”
lights on that Claim’s feat cascade. Idle and ample-fuel full-window Claims
leave it Dark. Idle Claim stays idle. Ample-fuel Claim
keeps the action when it did not halt.
...
| Hunt → Camp loop | two crafts, three tracks, Hand wear | Fogwort ×2 → Lamp-oil (`press-lamp-oil`) and Fogwort + Pale-cap + Tinderscrap → Fog-hood (`stitch-fog-hood`, `slot: head`). Instant atomic pay like the oil press — not a 25-recipe Chandlercraft skill, not the 2×3 wear grid. Keeper’s Camp still has three tracks; Lantern & Wick now divides Hunt `playerOffense.speedMs` the same way it shortens gathering bars. Hand on Camp reuses `equipWeapon` (wick-knife / unarmed). No 184-recipe dump, no six-slot doll. |
...
| Lantern-loaf | +14 HP | Eat-now-or-one-more-hit. A successful Eat in a live fight **restarts the player's swing** (`nextActMs = weapon speed`) so loaf cannot ride a filling strike bar; a second Eat also restarts it; that Eat cannot land a player blow on the same tick. Foe timer and Fall back are untouched. The fight paints **one** selected food (name, **+heal constant**, count, one Eat). Tap the slot to cycle owned foods; with one owned food the slot is a label, not an armed pick. Eat greys at full HP; never paint `+0` for an owned item (pending heal is the HP delta, not the label). Fall back sits on the eat row. Pale-cap +8 and fogwort +5 are forageable — gathering feeds combat. |
| After a hunt | leftover cockpit | Same two-pane fight as live: You vs last foe HP, Acc/they, oil (**Need oil** once when dry, plus a stall **Wick-oil ✦8** buy on this page), eat, Knife/Unarmed, Strike/Shot/Rite, Hunt-same-foe, Hunt another, 4-line log pinned above the 360 tab bar (`logWrap.bottom` and every `.log-line` bottom < tab top 577). Leftover wrap is **100px** so a wrapping kill + wrapping oil still fit four rows; fight wrap stays 64px. `.cockpit-fill` consumes leftover flex space so the loot **tray** cannot shove the log into the tab. Loot is a held collect pile (≤8 stacked chips + **Take all**) that persists across Hunt-this-foe and Fall back until Take all or Hunt another. Kill does **not** bank lumen/souls/items — `lootTray[].granted === false` until collect. Take all pays once (HUD lumen jumps); Hunt another auto-collects first then returns the zone hunt list **only when the tray is empty**. If the hollow is full, unbanked chips stay ungranted on leftover — Take all / Hunt another must not wipe them. Ungranted chips stay painted on the **live fight** after Hunt-this-foe (Melvor Loot-to-Collect stays up during the next pull) — Eat / Fall back / Strike nodes must survive ticks with that tray mounted. Live `.fight-loot` is a **32px** compact row (not leftover-actions 44px) and live Keep hunting is **32px** so unpaid chips sit ≥8px above tab 577; v54 compact 32px from craft-nav sat 550.6–582.6 (5.6px over SKILLS). Leftover Hunt-another stays 44px. A kill-log line may name loot as flavour; the wallet waits. Leftover is a combat page — hunt list / Vigil / Stretches are unmounted (not a lobby sticker). Leftover Hunt stays `Hunt Pale Moth` when dry (HTML `disabled` + `aria-disabled`) — never a second Need oil; the oil buy is the door. Stretch Hunt buttons stay `Hunt` (aria-disabled only, not HTML disabled). First lobby Hunt Pale Moth sits on the card head, above the fold. Kicker is “X fell” / “Fell back from X”. Hunt another sits on leftover-actions with the tray (not the eat-slot) so it does not clip past 360px. |

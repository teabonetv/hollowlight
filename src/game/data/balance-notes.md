...
| After a hunt | leftover station | Hub keeps You HP, **live** Acc/they vs last foe (moves with Knife ↔ Unarmed and Strike/Shot/Rite, interval on the same row), oil sips (**Need oil** in-frame at 0), eat, loot (✦ / souls / item), and Hunt-same-foe. No duplicate `N souls · lantern dry · 0 sips` row under the card. Kicker is “X fell” on a kill and “Fell back from X” on flee. Fight log is scoped to the current Hunt (cleared on startFight). Full Hand advert waits until the next stretch pick. |

...
| After a hunt | leftover cockpit | Same two-pane fight as live: You vs last foe HP, Acc/they, oil (**Need oil** once when dry), eat, Knife/Unarmed, Strike/Shot/Rite, Hunt-same-foe, 4-line log pinned above the 360 tab bar (`log bottom < tab top`). Vigil / Stretches wait below that frame. Leftover Hunt stays `Hunt Pale Moth` when dry (disabled) — never a second Need oil. Stretch Hunt buttons stay `Hunt`, not Need oil. First lobby Hunt Pale Moth sits on the card head, above the fold. Kicker is “X fell” / “Fell back from X”. |

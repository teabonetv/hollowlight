...
| Accuracy | 8 + 2×level + weapon | vs moth avoidance 11 ≈ 61% hit at level 1 with the wick-knife — misses matter. Live cockpit shows Acc% and they% as paired chips on one line (`Acc 61% · 4–7 / they 53% · 1–3`). Hunt from a scrolled hub resets `#screen` so that line is the first 360 frame. |
...
| Lantern-loaf | +14 HP | Eat-now-or-one-more-hit. The fight paints **one** selected food (name, **+heal constant**, count, one Eat). Tap the slot to cycle owned foods. Eat greys at full HP; never paint `+0` for an owned item (pending heal is the HP delta, not the label). Pale-cap +8 and fogwort +5 are forageable — gathering feeds combat. |
| After a hunt | leftover station | Hub keeps You HP, **live** Acc/they vs last foe (moves with Knife ↔ Unarmed and Strike/Shot/Rite), oil sips (**Need oil** in-frame at 0), and the selected food. Kicker is “X fell” on a kill and “Fell back from X” on flee. A tapped Hunt clears the log; Keep hunting keeps the kill loot/XP line into the next fight. Full Hand advert waits until the next stretch pick. |

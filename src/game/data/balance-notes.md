...
| After a hunt | leftover station | Hub keeps You HP, **live** Acc/they vs last foe (moves with Knife ↔ Unarmed and Strike/Shot/Rite, interval on the same row), oil sips (**Need oil** in-frame at 0), eat, Knife/Unarmed + styles, loot (✦ / souls / item), Hunt-same-foe, and a 4-line fight log (pinned on `lastStation.log`, never `log: []` after kill/flee). No duplicate `N souls · lantern dry · 0 sips` row under the card. Kicker is “X fell” on a kill and “Fell back from X” on flee. A new Hunt still clears live fight lines (`startFight`). Full Hand advert waits until the next stretch pick. |

...
/** Visible leftover collect pile. Stacks same kind; idle can still read it. */
export const LEFTOVER_TRAY_CAP = 8;
...
const STARTER_WEAPON_ORDER = ['wick-knife', 'ash-sling', 'prayer-stub'];

function defaultWeaponId(state) {
  for (const id of STARTER_WEAPON_ORDER) {
    if (bank.bankCount(state.bank, id) > 0) return id;
  }
  return null;
}
...
export function ownedWeapons(state) {
  ensureCombat(state);
  return Object.values(WEAPON_BY_ID).filter((w) => bank.bankCount(state.bank, w.id) > 0);
}

export function oilSipsRemaining(state) {
  let n = 0;
...
export function pauseCombat(state) {
  ensureCombat(state);
  if (state.combat.fighting) state.combat.paused = true;
}
...
export function heldWeapon(state) {
  ensureCombat(state);
  const id = state.combat.equipment?.weapon;
  const w = id ? WEAPON_BY_ID[id] : null;
  if (w && bank.bankCount(state.bank, id) > 0) return w;
  return null;
}

export function equipWeapon(state, itemId) {
  ensureCombat(state);
  if (itemId == null || itemId === 'unarmed') {
    state.combat.equipment.weapon = null;
    if (state.combat.fighting) {
      ...
      pushCombatLog(state, 'You fight unarmed.', 'style');
    }
    return { ok: true, weapon: null };
  }
  const w = WEAPON_BY_ID[itemId];
  if (!w) return { ok: false, error: 'Unknown weapon.' };
  if (bank.bankCount(state.bank, itemId) <= 0) return { ok: false, error: 'You do not hold that.' };
  state.combat.equipment.weapon = itemId;
  state.combat.player.style = w.style;
  if (state.combat.fighting) {
    const off = playerOffense(state, w.style);
    state.combat.player.nextActMs = Math.min(state.combat.player.nextActMs, off.speedMs);
    const name = ITEMS_BY_ID[itemId]?.name ?? itemId;
    pushCombatLog(state, `You take up the ${name}.`, 'style');
  }
...
export function equippedWeapon(state, style) {
  ensureCombat(state);
  const w = heldWeapon(state);
  if (w && w.style === style) return w;
...
function copyTrayEntry(e) {
  return {
    kind: e.kind,
    id: e.id ?? null,
    qty: e.qty,
    name: e.name ?? null,
    granted: e.granted !== false,
  };
}
...
export function pushLeftoverTray(state, entries) {
  const c = ensureCombat(state);
  for (const raw of entries ?? []) {
    if (!raw || !(raw.qty > 0)) continue;
    const e = copyTrayEntry(raw);
    const hit = c.lootTray.find((t) => trayMatch(t, e));
    if (hit) {
      hit.qty += e.qty;
      if (e.granted === false) hit.granted = false;
    } else {
      c.lootTray.push(e);
    }
  }
  if (c.lootTray.length > LEFTOVER_TRAY_CAP) {
    c.lootTray.splice(0, c.lootTray.length - LEFTOVER_TRAY_CAP);
  }
  return c.lootTray;
}
...
export function eatHealAmount(state, itemId) {
  ensureCombat(state);
  const food = FOOD[itemId];
  if (!food) return 0;
  const room = Math.max(0, playerMaxHp(state) - state.combat.player.hp);
  return Math.min(food.heal, room);
}
...
function categoriesOnStretch(zoneId) {
  const ids = [];
  for (const e of enemiesInZone(zoneId, { bosses: false })) {
    if (!ids.includes(e.category)) ids.push(e.category);
  }
  return ids;
}

export function assignVigil(state, { categoryId, seed, zoneId } = {}) {
  ensureCombat(state);
  if (state.combat.vigils.current) return { ok: false, error: 'A Vigil is already sworn.' };
  const requested = zoneId ?? state.combat.zoneId ?? 'hearthway';
  const stretchId = zoneUnlock(state, requested).ok ? requested : 'hearthway';
  const present = categoriesOnStretch(stretchId);
  const pool = VIGIL_CATEGORIES.filter((cat) => present.includes(cat.id));
  const fallback = pool[0] ?? VIGIL_CATEGORY_BY_ID.pale;
  const rng = createRng(seed ?? hashSeed(`vigil|${state.stats.playtimeMs}|${state.combat.vigils.nextTier}`));
  let cat;
  if (categoryId && present.includes(categoryId)) cat = VIGIL_CATEGORY_BY_ID[categoryId];
  else if (categoryId && VIGIL_CATEGORY_BY_ID[categoryId] && !present.includes(categoryId)) {
    // Tests (and later beacons) may pin a category; still bind the current stretch.
    cat = VIGIL_CATEGORY_BY_ID[categoryId];
  } else if ((state.combat.vigils.nextTier ?? 1) <= 1 && present.includes('pale')) {
    cat = VIGIL_CATEGORY_BY_ID.pale;
  } else {
    cat = pool.length ? rng.pick(pool) : fallback;
  }
...
function progressVigil(state, enemy) {
  ...
  if (v.zoneId && enemy.zoneId !== v.zoneId) return [];
  v.kills += 1;
...
export function tickCombat(state, dtMs) {
  ...
  const events = [];
  const c = state.combat;
  if (!c.fighting || !c.foe || c.paused) return events;

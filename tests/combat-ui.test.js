...
test('leftover log-wrap CSS is 96px+ and leftover-live does not scroll the lobby', () => {
  ...
  const wideFight = css.match(
    /@media \(min-width: 900px\)\s*\{[\s\S]*?#screen:has\(\.fight-live\),\s*\n\s*#screen:has\(\.leftover-live\)\s*\{([^}]+)\}/,
  );
  assert.ok(wideFight, '900px fight/leftover padding override');
  assert.match(wideFight[1], /padding:\s*8px 16px calc\(8px \+ env\(safe-area-inset-bottom/);
  assert.doesNotMatch(wideFight[1], /--tabbar-size/);
